...
import training.TrainingGUI;
import ai.Player;
import game.Backgammon;
import game.DiceRoll;


public class VsGUI extends Application {

	public static VsGUI currentGUI;
	
	static final String playerFile = "bigBad";
	...
	public void start(Stage stage) {
		...
		{ // Code for Start Scene ----------------------------------------------
			...
			Button startButton = new Button("Start");
			{
				...
				startButton.setPrefSize(400, 250);
				startButton.setOnAction(e -> {
					if (whiteText.getText().isBlank()) {
						whitePlayer = null;
					} else {
						whitePlayer = Player.loadPlayer(whiteText.getText() + ".txt");
					}
					if (blackText.getText().isBlank()) {
						blackPlayer = null;
					} else {
						blackPlayer = Player.loadPlayer(blackText.getText() + ".txt");
					}
					game = new Backgammon();
					gameStage.show();
					startStage.hide();
			...
			HBox selectionArea = new HBox();
			{
				...
				whiteText = new TextField("");
				whiteText.setAlignment(Pos.CENTER);
				whiteBox.setAlignment(Pos.CENTER);
				whiteBox.getChildren().add(whiteLabel);
				whiteBox.getChildren().add(whiteText);

				VBox blackBox = new VBox(5);
				Label blackLabel = new Label("Black Player");
				blackText = new TextField(playerFile);
				blackText.setAlignment(Pos.CENTER);
				
				blackBox.setAlignment(Pos.CENTER);
				blackBox.getChildren().add(blackLabel);
				blackBox.getChildren().add(blackText);

				selectionArea.getChildren().add(whiteBox);
				selectionArea.getChildren().add(blackBox);
				selectionArea.setPrefSize(400, 10);
				selectionArea.setSpacing(100);
				selectionArea.setPadding(new Insets(0, 50, 0, 50));
				selectionArea.setAlignment(Pos.TOP_CENTER);
				BorderPane.setMargin(selectionArea, new Insets(30, 0, 30, 0));
			}
			// Adding Elements to the startPane
			{
				startPane.setTop(welcomeLabel);
				startPane.setBottom(selectionArea);
		...
		{ // Code for Game Scene
			...
			// Creating move AI Button
			Button aiButton = new Button();
			aiButton.setText("Move AI");
			aiButton.setPrefSize(100, 50);
			aiButton.setOnAction(e-> {
				requestNextMove();
			});
			gameGroup.getChildren().add(aiButton);
			aiButton.setLayoutY(500);
			aiButton.setLayoutX(400);
	...
	void requestNextMove() {
		int[] bestMove = null;
		if (game.getWhiteTurn() && whitePlayer != null) {
			bestMove = whitePlayer.getBestMove(false);
		} else if (!game.getWhiteTurn() && blackPlayer != null) {
			bestMove = blackPlayer.getBestMove(false);
		}
		if (bestMove == null) {
			game.switchTurnIfNoMoves();
		} else {
